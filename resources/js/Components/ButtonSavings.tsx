...
export default function ButtonSecondary({
    ...
    const handleBounce = () => {
        if (isBouncing) return; // アニメーション中は無視
        setIsBouncing(true);
        setTimeout(() => setIsBouncing(false), 500); // アニメーション後にクラスを削除
    };

    const handleClick = (event: React.MouseEvent<HTMLButtonElement>) => {
        handleBounce();
        if (props.onClick) {
            props.onClick(event); // 親コンポーネントのonClickイベントを呼び出す
        }
    };
    ...
    return (
        <button
            ...
            disabled={disabled}
        >
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 512 512" className={`object-contain ${isBouncing ? 'animate-bounceSvg' : ''}`}>
